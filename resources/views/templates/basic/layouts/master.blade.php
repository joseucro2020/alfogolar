...
<head>
    ...
    <link rel="stylesheet" href="{{ asset($activeTemplateTrue.'css/placeholder.css') }}">
    <link rel="stylesheet" href="{{ asset($activeTemplateTrue.'css/product-item.css?v=0.0.3') }}">
    <link rel="stylesheet" href="{{ asset($activeTemplateTrue.'css/category-slider.css?v=0.0.3') }}">
...
<script type="text/javascript">
    ...
    s1.async=true;
    s1.src='https://embed.tawk.to/62a2132e7b967b117993b590/1g54hlrb3';
    s1.charset='UTF-8';
    ...
    <!--End of Tawk.to Script-->
        ...
        @include($activeTemplate.'layouts.placeholderloading')
            ...
            */
            ...
            window.addEventListener("scroll", () => {
                ...
                scrollTimeout = setTimeout(() => {
                    let container = document.getElementById("show_categories_products");
                    let show_products_for_categories = document.getElementById("show_products_for_categories");
                    let show_products_search = document.getElementById("show_products_search");
                    let show_products_offers = document.getElementById("show_products_offers");
                    ...
                    if (footerCoor.bottom - viewportSize <= footerCoor.height) {
                        ...
                        /*Listado de carrusel de categorias y productos*/
                        if(container){
                            $.ajax({
                                ...
                                success: function(response){
                                    if (response.length > 47) {
                                        ...
                                        $('#show_categories_products').append(response);
                                        $('#msg_loading').hide();
                                        pageAct = page;
                        ...
                        if(show_products_for_categories){
                            let id = $("#idCategory").data("id")
                            $.ajax({
                                url: "{{ route('more_products_category') }}",
                                method: "get",
                                data:{
                                    page: page, 
                                    id: id 
                                ...
                                success: function(response){
                                    console.log(response.length);
                                    if (response.length > 107) {
                                        $('#show_products_for_categories').append(response);
                                        $('#msg_loading').hide();
                                        pageAct = page;
                        ...
                        /*Listado de Productos por Busqueda*/
                        if(show_products_search){
                            let id = $("#idCategory").data("id")
                            let search = $("#search").data("search")

                            $.ajax({
                                url: "{{ route('more_product_search') }}",
                                method: "get",
                                data:{
                                    page: page, 
                                    id: id,
                                ...
                                success: function(response){
                                    console.log(response.length);
                                    if (response.length > 107) {
                                        $('#show_products_search').append(response);
                                        $('#msg_loading').hide();
                                        pageAct = page;
                        ...
                        /*Listado de Productos Ofertas*/
                        if(show_products_offers){
                            $.ajax({
                                url: "{{ route('more_product_offers') }}",
                                method: "get",
                                data:{
                                    page: page
                                },
                                success: function(response){
                                    console.log(response.length);
                                    if (response.length > 107) {
                                        $('#show_products_offers').append(response);
                                        $('#msg_loading').hide();
                                        pageAct = page;
        ...
        function make_skeleton(){
            ...
            for(var count = 0; count < 15; count++){
                for(var count1 = 0; count1 < 5; count1++){
                    output += '<div class="col-12 col-sm-2">';
                    ...
                    output += '<div class="ph-col-4 big empty"></div>';
                    output += '</div>';
                    output += '</div>';
                    output += '</div>';
                    output += '</div>';
            ...
            $.ajax({
                ...
                success: function(response){
                   ...
                   if (response.error) {
                    ...
                    //if (response.length > 0) {
                        vue.usershipping = response.shipping
                        //$('#shipping_user').html(response);
                        //console.log(response);
                        // AddressUser(
