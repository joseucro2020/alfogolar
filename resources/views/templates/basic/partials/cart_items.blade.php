                    ...
                    @php
                        if($item->attributes != null){
                            ...
                            if($item->product->offer && $item->product->offer->activeOffer){
                                ...
                                if($item->is_prime == 1){
                                    $s_price = $item->product->precioPrimeIva > 0 ? $item->product->precioPrimeIva : $item->product->precioBaseIva;
                                    if(session()->get('moneda') == 'Dolares'){
                                        echo getAmount($s_price, 2);
                                    }
                                    else{
...
@if($subtotal > 0)
    <div class="d-flex justify-content-between mt-3 x-finalizar">
        ...
        @else  
            <span class="text-argo-total"> @lang('Subtotal') Bs {{ getAmount($subtotal * $rate, 2) }}</span>
        @endif
        @if($data->count()>0)
    ...
    @if($tasa->count() > 0)  
        <div class="d-flex justify-content-between">
            <span class="text-argo-total"> @lang('Tasa') {{ getAmount($tasa->tasa_del_dia, 2)}} Bs </span>
        </div>
    @endif  
    <div class="d-flex justify-content-between">
        <span class="text-argo-total"> @lang('Total') {{ getAmount($total, 2) }} Bs </span>
    </div>           
