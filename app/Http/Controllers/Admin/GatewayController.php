...
class GatewayController extends Controller
{
    ...
    {
        $page_title = 'Pasarelas de Pago Automáticas';
        $empty_message = 'No se ha instalado ninguna puerta de enlace.';
        $gateways = Gateway::orderBy('status','DESC')->orderBy('name','ASC')->with('currencies')->get();
      //  dd($gateways);
    ...
    {
        $gateway = Gateway::with('currencies')->where('alias', $alias)->firstOrFail();
        $page_title = 'Pasarela Actualizada : ' . $gateway->name;
    ...
    public function update(Request $request, $code)
    {
        ...
        if ($request->hasFile('image')) {
            try {
                ...
                $notify[] = ['errors', 'No se pudo cargar la imagen.'];
            ...
            foreach ($request->currency as $key => $currency) {
                ...
                if ($request->hasFile($uploaded_image)) {
                    try {
                        ...
                        $notify[] = ['error', $currency_identifier . ' No se pudo cargar la imagen.'];
                        return back()->withNotify($notify);
                    }
                }
        ...
        $notify[] = ['success', $gateway->name . ' Ha Sido Actualizado.'];
        return redirect()->route('admin.deposit.gateway.edit', $gateway->alias)->withNotify($notify);
    }
    ...
    public function remove(Request $request, $code)
    {
        ...
        $notify[] = ['success', $name . ' Ha Sido Removido de ' . $gateway->name];
        return back()->withNotify($notify);
    }
    ...
    public function activate(Request $request)
    {
        ...
        $notify[] = ['success', $gateway->name . ' Ha Sido Activado.'];
        return back()->withNotify($notify);
    }
    ...
    public function deactivate(Request $request)
    {
        ...
        $notify[] = ['success', $gateway->name . ' Ha Sido Desabilitado.'];
        return back()->withNotify($notify);
    }
